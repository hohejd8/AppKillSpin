#include "AddStrahlkorperSurfaceAndMesh.hpp"
...
namespace DataBoxAdders {
  ...
  void AddStrahlkorperSurfaceAndMesh::
  AddToDataBoxImpl(const DataBoxInserter& boxi,const std::string& Opts) const
  {
    ...
    const StrahlkorperMesh skm(mNth, mNph);

    boxi.AddConstItem(mSurface, theStrahlkorper);
    boxi.AddConstItem(mMesh, skm);
  }
}
